...
# He isn't fond of seeing the Eiffel tower 40 times. He tells them that he will only sit 
# during the session if they show the same motive at most N times. Luckily, Alice and Bob 
# are able to encode the motive as a number. Can you help them to remove numbers such that 
...
def delete_nth(lst, n)
  new_lst = []
  lst.each { |num| new_lst << num if new_lst.count(num) < n }
  new_lst
end
